<?php
if(!defined('RESPONSE_SUCCESS')) define('RESPONSE_SUCCESS', 200);
if(!defined('RESPONSE_UNAUTHORIZED')) define('RESPONSE_UNAUTHORIZED', 401);
if(!defined('RESPONSE_UNPROCESSABLE_ENTITY')) define('RESPONSE_UNPROCESSABLE_ENTITY', 422);

// message constant
if(!defined('RESPONSE_ALL_ROWS')) define('RESPONSE_ALL_ROWS', 'all row fetched.');
if(!defined('RESPONSE_ADD_ROWS')) define('RESPONSE_ADD_ROWS', 'added successfully.');
if(!defined('RESPONSE_EDIT_ROWS')) define('RESPONSE_EDIT_ROWS', 'edit successfully.');